...
function executePriceUpdate(update: PriceUpdate, logger: Logger) {
    logger.info("Starting price update executions for update: " + update.name);
    update.contractUpdates.forEach(contractUpdate => {
        try {
            ...
            logger.info("Transaction sent for chainId: " + contractUpdate.chainId + " txHash: " + tx.hash);
            const receipt = await tx.wait();
            logger.info("Result of transaction is: " + receipt.status);
        }   catch (e) {
            logger.error("Error executing price update for chainId: " + contractUpdate.chainId + " error: " + e);
        }
    }
}